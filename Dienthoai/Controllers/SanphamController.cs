...
namespace Dienthoai.Controllers
{
    public class SanphamController : Controller
    {
        ...
        {
            var ip = db.Sanphams.Where(n => n.Mahang == 2).Take(18).ToList();
            return PartialView(ip);
        }
        public ActionResult dtsamsungpartial()
        {
            var ss = db.Sanphams.Where(n => n.Mahang == 1).Take(18).ToList();
            return PartialView(ss);
        }
        public ActionResult dtxiaomipartial()
        {
            var mi = db.Sanphams.Where(n => n.Mahang == 3).Take(18).ToList();
